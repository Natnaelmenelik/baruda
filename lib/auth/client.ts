...
//   }
// }
const ALL_AUTH_KEYS_TO_CLEAR = [
  "token",
  "user",
  "session",

  // old / duplicate keys to remove during logout
  "auth_token",
  "access_token",
  "refresh_token",
  "auth_user",
...
const AUTH_COOKIE_KEYS_TO_CLEAR = [
  "token",
  "auth_token",
  "access_token",
  "refresh_token",
...
export function setClientSession(session: any) {
  ...
  try {
    const logoutLocked =
      localStorage.getItem(LOGOUT_IN_PROGRESS_KEY) === "true";
    const isLoginPage = window.location.pathname.includes("/login");
  ...
  try {
    ...
    // Remove old duplicate localStorage keys after successful login.
...
export function clearClientSession() {
  ...
  try {
    ...
    for (const key of ALL_AUTH_KEYS_TO_CLEAR) {
      try {
        localStorage.removeItem(key);
      } catch {}
    }
    ...
    for (const key of AUTH_COOKIE_KEYS_TO_CLEAR) {
      expireCookie(key);
    }
