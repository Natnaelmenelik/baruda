...
export default function MinsamPage() {
  ...
  const [returnPendingTarget, setReturnPendingTarget] = useState<any | null>(null);
  ...
  const openReturnPendingConfirm = (sub: any) => {
    setReturnPendingTarget(sub);
  };

  const confirmReturnPending = async () => {
    if (!returnPendingTarget) return;

    const id =
      returnPendingTarget.id ||
      returnPendingTarget.submission_group_id ||
      returnPendingTarget.primary_id;

    try {
      setReturnPendingLoading(true);
      setProcessingSubmissionId(id);

      const res = await fetch(`/api/minsam/submissions/${id}/return-pending`, {
        method: "POST",
      ...
      if (!res.ok) {
        throw new Error(
          data?.error ||
            (txt.failedToReturnPending || "Failed to return submission to pending"),
        );
      }

      toast.success(txt.returnedToPending || "Submission returned to pending");
  ...
  return (
    <div className="min-h-screen p-4 bg-gray-100 md:p-6">
      <div className="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold">{txt.minsamPanel || 'Minsam Panel'}</h1>
...
<AdminNumbersPanel />
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              {(selectedSubmission.status === "approved" ||
                ...
                <button
                  type="button"
                  onClick={() => openReturnPendingConfirm(selectedSubmission)}
                  disabled={returnPendingLoading}
                  className="rounded-xl bg-yellow-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-yellow-600 disabled:opacity-50"
                >
                  {txt.returnPending || "Return Pending"}
                </button>
              )}
      ...
      {returnPendingTarget && (
        <Modal onClose={() => setReturnPendingTarget(null)}>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            {txt.returnPendingConfirmTitle || "Return to Pending?"}
          </h2>

          <p className="mt-3 text-sm text-gray-600 dark:text-slate-300">
            {txt.returnPendingConfirmMessage ||
          ...
          <div className="mt-4 rounded-xl bg-yellow-50 p-3 text-sm text-yellow-900">
            <div className="flex justify-between gap-3">
              <span>{txt.user}</span>
              <b>{returnPendingTarget.user_name || txt.unknown || "-"}</b>
            </div>
            <div className="mt-2 flex justify-between gap-3">
              <span>{txt.amount}</span>
              <b>
                {Number(returnPendingTarget.total_amount || 0).toLocaleString()} {txt.birr}
              </b>
            </div>
            <div className="mt-2 flex justify-between gap-3">
              <span>{txt.status}</span>
              <b>{statusLabel(returnPendingTarget.status)}</b>
            </div>
          </div>
          ...
          <div className="mt-6 flex gap-3">
            <button
              type="button"
              onClick={() => setReturnPendingTarget(null)}
              disabled={returnPendingLoading}
              className="flex-1 rounded-xl border px-4 py-3 font-semibold text-gray-700 dark:text-slate-200 disabled:opacity-50"
            >
              {txt.cancel}
            </button>
            ...
            <button
              type="button"
              onClick={confirmReturnPending}
              disabled={returnPendingLoading}
              className="flex-1 rounded-xl bg-yellow-500 px-4 py-3 font-semibold text-white hover:bg-yellow-600 disabled:opacity-50"
            >
