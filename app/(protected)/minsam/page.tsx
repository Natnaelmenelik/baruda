...
export default function MinsamPage() {
  ...
  const returnToPendingLabel =
    lang === 'am' ? 'ወደ በመጠባበቅ መልስ' : 'Return to Pending';

  const returnToPendingSuccess =
    lang === 'am'
      ? 'ግቤቱ ወደ በመጠባበቅ ተመልሷል።'
      : 'Submission returned to pending.';

  const returnToPendingFailed =
    lang === 'am'
  ...
  const [returnPendingModalId, setReturnPendingModalId] = useState<string | null>(null);
  const [processingType, setProcessingType] = useState<'approve' | 'reject' | 'return-pending' | null>(null);
  ...
  const handleReturnToPending = (id: string) => {
    setReturnPendingModalId(id);
  };

  const confirmReturnToPending = () => {
    if (!returnPendingModalId) return;

    const id = returnPendingModalId;

    setReturnPendingModalId(null);
    setProcessingSubmissionId(id);
    setProcessingType('return-pending');

    returnToPending(id, {
      onSuccess: () => toast.success(returnToPendingSuccess),
      onError: (err: any) => {
        const message =
  ...
  const closeReturnPendingModal = () => {
    if (processingType === 'return-pending') return;
    setReturnPendingModalId(null);
  };
  ...
  return (
    <>
    <ReturnApprovedToPendingModal
      open={!!returnPendingModalId}
      lang={lang}
      isLoading={processingType === 'return-pending'}
      cancelLabel={txt.cancel}
      confirmLabel={returnToPendingLabel}
      onClose={closeReturnPendingModal}
                    ...
                    <tr key={sub.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3">
                        {sub.status === 'pending' ? (
                          ...
                          <button
                            onClick={() => handleReturnToPending(sub.id)}
                          ...
                          >
                            {processingSubmissionId === sub.id && processingType === 'return-pending' ? (lang === 'am' ? 'በመመለስ ላይ...' : 'Returning...') : returnToPendingLabel}
                          </button>
